...
class PatienIDViewController: UITableViewController {
    ...
    let dataFilePath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?.appendingPathComponent("Patients.plist")
...
//        if let items = defaults.array(forKey: "PatientListArray") as? [Item] {
//            itemArray = items
